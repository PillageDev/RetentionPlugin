 ...
 * @author desht (Original code), KingFaris10 (Editor of code), Pilage_ (Editor of code)
 */
public class Cuboid implements Iterable<Block>, Cloneable, ConfigurationSerializable {
    ...
    /**
     * Get the coordinates for the top and bottom of the perimeter
     *
     * @return List of coordinates for the top and bottom perimeter of the rectangle
     */
    public List<Location> getTopBottomFacePerimeter(double particleDistance) {
        Set<Location> edges = new HashSet<>();

        // bottom face
        for (double x = this.getLowerX(); x <= this.getUpperX(); x += particleDistance) {
            edges.add(new Location(this.getWorld(), x, this.getLowerY(), this.getLowerZ()));
            edges.add(new Location(this.getWorld(), x, this.getLowerY(), this.getUpperZ()));
        }

        for (double z = this.getLowerZ(); z <= this.getUpperZ(); z += particleDistance) {
            edges.add(new Location(this.getWorld(), this.getLowerX(), this.getLowerY(), z));
            edges.add(new Location(this.getWorld(), this.getUpperX(), this.getLowerY(), z));
        }

        // top face
        for (double x = this.getLowerX(); x <= this.getUpperX(); x += particleDistance) {
            edges.add(new Location(this.getWorld(), x, this.getUpperY(), this.getLowerZ()));
            edges.add(new Location(this.getWorld(), x, this.getUpperY(), this.getUpperZ()));
        }

        for (double z = this.getLowerZ(); z <= this.getUpperZ(); z += particleDistance) {
            edges.add(new Location(this.getWorld(), this.getLowerX(), this.getUpperY(), z));
            edges.add(new Location(this.getWorld(), this.getUpperX(), this.getUpperY(), z));
        }

        return new ArrayList<>(edges);
    }

    public List<Location> getVerticalEdgeLocations(double particleDistance) {
        List<Location> locations = new ArrayList<>();
        int lowestY = this.getLowerY();
        int highestY = this.getUpperY();

        Block[] blockCorners = this.corners();
        List<Location> corners = Arrays.stream(blockCorners).map(Block::getLocation).collect(Collectors.toList());

        List<Location> bottomCorners = corners.stream().filter(loc -> loc.getY() == lowestY).collect(Collectors.toList());

        for (Location bottomCorner : bottomCorners) {
            for (double y = lowestY; y <= highestY; y += particleDistance) {
                Location verticalLocation = bottomCorner.clone().add(0, y - bottomCorner.getY(), 0);
                locations.add(verticalLocation);
            }
        }
